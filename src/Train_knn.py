...
parser.add_argument('--eval-embedding', action='store_true', default=False,
                    help='Evaluate val accuracy/AUC using KNN on embeddings when only triplet')

# ===== THAM SỐ CHO KNN (thay thế SVM) =====
parser.add_argument('--knn-k', type=int, default=5,
                    help='Number of neighbors for KNN')
parser.add_argument('--knn-metric', type=str, default='euclidean',
...
# ============================================================
# ===== HÀM ĐÁNH GIÁ BẰNG KNN (THAY THẾ SVM) =====
# ============================================================
def evaluate_embedding_knn(model, train_loader, val_loader, device, 
                           n_neighbors=args.knn_k, metric=args.knn_metric):
    """
    Đánh giá chất lượng embedding bằng KNN trên validation set.
    """
    model.eval()
    ...
    # Huấn luyện KNN (lazy learning: chỉ lưu dữ liệu, không tính toán)
    knn = KNeighborsClassifier(n_neighbors=n_neighbors, metric=metric)
    knn.fit(X_train, y_train)  # Chỉ lưu dữ liệu train

    y_pred = knn.predict(X_val)
    y_proba = knn.predict_proba(X_val)[:, 1] if knn.classes_.shape[0] == 2 else None

    acc = accuracy_score(y_val, y_pred)
    auc = roc_auc_score(y_val, y_proba) if y_proba is not None else 0.0
    cm = confusion_matrix(y_val, y_pred)
    TN, FP = cm[0,0], cm[0,1] if cm.shape == (2,2) else (0,0)
    FN, TP = cm[1,0], cm[1,1] if cm.shape == (2,2) else (0,0)
    sens = TP / (TP + FN) if (TP + FN) > 0 else 0.0
    spec = TN / (TN + FP) if (TN + FP) > 0 else 0.0

    print(f'KNN (k={n_neighbors}, metric={metric}) on Val: Acc: {acc*100:.2f}%, AUC: {auc:.4f}, Sens: {sens:.4f}, Spec: {spec:.4f}')
    return acc * 100, auc, sens, spec


def evaluate_final_knn(model, train_loader, test_loader, device,
                       n_neighbors=args.knn_k, metric=args.knn_metric):
    """
    Đánh giá cuối cùng trên test set bằng KNN.
    """
    model.eval()
    train_embs, train_labels = [], []
    ...
    y_proba = knn.predict_proba(X_test)[:, 1] if knn.classes_.shape[0] == 2 else None
    ...
    print(f'Test set (KNN, k={n_neighbors}, metric={metric}): Accuracy: {acc:.2f}%, AUC: {auc:.4f}, Sens: {sens:.4f}, Spec: {spec:.4f}')
...
def train_with_margin(margin, args, train_loader, val_loader, test_loader, in_channels, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.only_triplet:
            if args.eval_embedding:
                # ===== SỬ DỤNG KNN THAY VÌ SVM =====
                val_acc, val_auc, val_sens, val_spec = evaluate_embedding_knn(
                    model, train_loader, val_loader, device,
                    n_neighbors=args.knn_k, metric=args.knn_metric
                )
                print(f'Val set (KNN): Accuracy: {val_acc:.2f}%, AUC: {val_auc:.4f}, Sens: {val_sens:.4f}, Spec: {val_spec:.4f}')
    ...
    if args.only_triplet:
        ...
        # ===== SỬ DỤNG KNN THAY VÌ SVM =====
        evaluate_final_knn(model_test, train_loader, test_loader, device,
                           n_neighbors=args.knn_k, metric=args.knn_metric)
    else:
