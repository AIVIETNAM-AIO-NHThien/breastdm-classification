...
class NLBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, mode='embedded',
        ...
        but doesn't include subsampling trick
        args:
        ...
        if bn_layer:
            ...
            # from section 4.1 of the paper, initializing params of BN ensures that 
            # the initial state of non-local block is identity mapping
            nn.init.constant_(self.W_z[1].weight, 0)
            nn.init.constant_(self.W_z[1].bias, 0)
        else:
            self.W_z = conv_nd(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1)

            # from section 3.3 of the paper by initializing Wz to 0, 
            # this block can be inserted to any existing architecture
...
# -------------------- Vision Transformer Base (nguyên bản tác giả) --------------------
...
# -------------------- FCUUp (Transformer patch embeddings -> CNN feature maps) --------------------
...
class FusionM(nn.Module):
    def __init__(self, num_classes=2, load_vit=False):
        ...
        # self.layer4 = model_se.layer4  # Không dùng layer4
        
        # FCUUp để chuyển ViT patch embeddings sang CNN feature maps
    ...
    def forward(self, x):
        # Nếu load ViT pretrained, load weight
        if self.load_true:
        ...
        # CNN path (SE-ResNet50)
        x = self.layer0(x)
        x = self.layer1(x)
        se_x = self.layer2(x)
        # se_x = self.layer3(x)  # Tác giả comment layer3
        # se_x = self.layer4(x)  # Tác giả comment layer4

        # Non-local fusion (cross-attention)
