...
parser.add_argument('--mining', type=str, default='hard', choices=['semi-hard', 'hard'],
                    help='Triplet mining strategy: semi-hard or hard')
...
# ============================================================
# ===== HARD TRIPLET MINING (thay thế Semi-Hard) =====
# ============================================================
def batch_hard_triplet_loss(embeddings, labels, margin):
    """
    Batch Hard Triplet Loss.
    Với mỗi anchor, chọn positive xa nhất và negative gần nhất.
    """
    ...
    positive_mask = labels == labels.t()  # (B, B) - True nếu cùng lớp
    negative_mask = ~positive_mask        # (B, B) - True nếu khác lớp
    
    # Loại bỏ self (anchor với chính nó)
    eye = torch.eye(embeddings.size(0), device=embeddings.device, dtype=torch.bool)
    positive_mask = positive_mask & ~eye
    negative_mask = negative_mask & ~eye
    
    # Hardest positive: khoảng cách xa nhất với cùng lớp
    hardest_positive_dist, _ = torch.max(pairwise_dist * positive_mask.float(), dim=1)
    
    # Hardest negative: khoảng cách gần nhất với khác lớp
    # Đặt khoảng cách của negative mask = 0 thành một giá trị rất lớn để bỏ qua
    neg_dists = pairwise_dist.clone()
    neg_dists[~negative_mask] = float('inf')
    hardest_negative_dist, _ = torch.min(neg_dists, dim=1)
    
    # Triplet loss: max(0, hardest_pos_dist - hardest_neg_dist + margin)
    loss = torch.relu(hardest_positive_dist - hardest_negative_dist + margin)
    
    # Nếu không có positive hoặc negative hợp lệ, loss = 0
...
def train_one_epoch(epoch, model, loader, optimizer, criterion_ce, criterion_triplet, device, args, margin):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        if args.only_triplet:
            ...
            loss = batch_hard_triplet_loss(embeddings, target, margin)
            ...
            if args.use_triplet:
                embeddings = model(data, return_embedding=True)
                loss_triplet = batch_hard_triplet_loss(embeddings, target, margin)
...
def train_with_margin(margin, args, train_loader, val_loader, test_loader, in_channels, device):
    print(f"\n{'='*60}")
    print(f"   BẮT ĐẦU HUẤN LUYỆN VỚI MARGIN = {margin} (Hard Mining)")
    print('='*60)
